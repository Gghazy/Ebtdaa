...
namespace Ebtdaa.Application.ActualRawMaterials.Handlers
{
    public class ActualRawFileService : IActualRawFileService
    {
        ...
        public async Task<BaseResponse<List<ActualRawFileResultDto>>> GetByItem(int id)
        {
            var respose = _mapper.Map<List<ActualRawFileResultDto>>(await _dbContext.ActualRawMaterialFiles.Where(x=>x.Id ==id).Include(x => x.Attachment).ToListAsync());
